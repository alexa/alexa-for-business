...
/// Key for the root node value containing configuration values for DeviceInfo.
static const std::string DEVICE_INFO_CONFIG_KEY("deviceInfo");

/// Register with A4B flag under @c SAMPLE_APP_CONFIG_KEY configuration node.
static const std::string REGISTER_WITH_A4B("registerWithA4B");

/// AVS device maker endpoint under @c SAMPLE_APP_CONFIG_KEY configuration node.
static const std::string DEVICE_MAKER_SERVICE_ENDPOINT("deviceMakerServiceEndpoint");

/// ProductId under @c DEVICE_INFO_CONFIG_KEY configuration node.
static const std::string PRODUCT_ID("productId");

/// DeviceSerialNumber under @c DEVICE_INFO_CONFIG_KEY configuration node.
static const std::string DEVICE_SERIAL_NUMBER("deviceSerialNumber");

/// ClientId under @c DEVICE_INFO_CONFIG_KEY configuration node.
static const std::string CLIENT_ID("clientId");

/// Default value for configured requestTimeout.
static const std::chrono::seconds DEFAULT_REQUEST_TIMEOUT = std::chrono::seconds(60);
...
void UIManager::onRequestAuthorization(const std::string& url, const std::string& code) {
    m_executor.submit([this, url, code]() {
        ...
        bool registerWithA4B = true;
        config[SAMPLE_APP_CONFIG_KEY].getBool(REGISTER_WITH_A4B, &registerWithA4B, true);
        if (registerWithA4B) {
            std::string deviceMakerServiceEndpoint;
            config[SAMPLE_APP_CONFIG_KEY].getString(DEVICE_MAKER_SERVICE_ENDPOINT, &deviceMakerServiceEndpoint, "");
            oss << "\nRegistering device with Alexa for Business";
            oss << "\nAVSdevice maker service endpoint: " << deviceMakerServiceEndpoint;
            std::string clientId;
            config[DEVICE_INFO_CONFIG_KEY].getString(CLIENT_ID, &clientId, "");
            oss << "\nClientId: " << clientId;

            std::string productId;
            config[DEVICE_INFO_CONFIG_KEY].getString(PRODUCT_ID, &productId, "");
            oss << "\nProductId: " << productId;

            std::string deviceSerialNumber;
            config[DEVICE_INFO_CONFIG_KEY].getString(DEVICE_SERIAL_NUMBER, &deviceSerialNumber, "");
            oss << "\nDeviceSerialNumber: " << deviceSerialNumber;

            oss << "\nUser Code -> " << code;

            std::shared_ptr<avsCommon::utils::libcurlUtils::HttpPostInterface> httpPost = avsCommon::utils::libcurlUtils::HttpPost::create();
            std::string body = "{\"Code\":\"" + code
                        + "\",\"ClientId\":\"" + clientId
                        + "\",\"ProductId\":\"" + productId
                        + "\",\"DSN\":\"" + deviceSerialNumber
                        +"\"}";
            try {
                const std::vector<std::string> headerLines = {HEADER_LINE_URLENCODED};
                auto response = httpPost->doPost(deviceMakerServiceEndpoint, headerLines, body, DEFAULT_REQUEST_TIMEOUT);
