...
1. **Test the Help Desk skill with your Echo device.**  Because your skill uses the resolveRoom API, the device association is needed in order to determine the room location from the calling user.  Therefore you'll need to use the actual Echo (shared device) in order to succesfully retrieve the room location.

2. **If you've forgotten the invocation name** for your skill, revisit the **Build** panel on the top navigation menu in the ASK Developer Console, and select Invocation from the sidebar to review it.

3. **Ensure your skill works the way that you designed it to.**
	* If it's not working as expected, you can look at the logs for your Lambda function, found in the Monitoring section of your function, and clicking on "Jump to Logs."  Adding debug console.log statements is one way of narrowing down any issues.

4. **If you still are having issues, consider hard coding a response for resolveRoom.**  It may be beneficial if you're having issues, to isolate the cause of any issues, by replacing the call to resolveRoom with a hard coded result, and checking the rest of the skill.  Note that you can change the Lambda code any way you like, without having to re-publish the skill.

 You can follow the steps below to troubleshoot your skill once the resolveRoom call is removed.
 
5.  **Go back to the [Amazon Developer Portal](https://developer.amazon.com/edw/home.html#/skills/list) and select your skill from the list.** You may still have a browser tab open if you started at the beginning of this tutorial.

6. Open the **Test** Pane, by selecting the **Test** link from the top navigation menu.

7. Enable Testing by activating the **Test is enabled for this skill** slider. It should be underneath the top navigation menu.

8. To validate that your skill is working as expected, invoke your skill from the **Alexa Simulator**. You can either type or click and hold the mic from the input box to use your voice.
...
9. 	After you interact with the Alexa Simulator, you should see the Skill I/O **JSON Input** and **JSON Output** boxes get populated with JSON data. You can also view the **Device Log** to trace your steps.
...
10.  **Configure a test event in AWS Lambda.** Now that you are familiar with the **request** and **response** boxes in the Service Simulator, it's important for you to know that you can use your **requests** to directly test your Lambda function every time you update it.  To do this:
...
11.  **Other testing methods to consider:**
...
12.  **If your sample skill is working properly, you can now customize your skill.**
